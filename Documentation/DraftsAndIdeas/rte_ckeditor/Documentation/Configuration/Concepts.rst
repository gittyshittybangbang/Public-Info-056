...
has everything preset.


.. tip::

   Before you start, have a look at the :ref:`configuration-best-practices`.
...
can be set via :ref:`t3tca:columns-text-properties-richtextConfiguration`, setting it to “news”.
...
The option `RTE.usePreset = news` can also be set on a per-field and
per-type basis:

.. code-block:: typoscript

   RTE.tt_content.types.bullets.bodytext.usePreset = bullets

This sets the "bullet" preset for all bodytext fields of content elements with
Content Type “Bullet list” (CType=bullets).
...
Links were not set with a <a> tag, but with a so-called <typolink 23,13 _blank> tag. Further
tags were <typolist> and <typohead>, which were stored in the database 1:1. Since RTEs did
not understand these special tags, they had to transform these special tags into valid HTML
tags. Additionally, TYPO3 did not store regular <p> or <div> tags but treated every line
without a surrounding HTML block element as <p> tag. The frontend rendering then added
<p> tags for each line when parsing (see below).

Transformations were later used to allow <em>/<strong> tags instead of <b>/<i> tags,
...
For TYPO3 v8, the `<typolink>` tag was migrated to proper `<a>` tags with a special
`<a href=”t3://page?id=23”>` syntax when linking to pages to ensure HTML valid output.
...
Most logic related to transformations can be found within :file:`TYPO3\CMS\Core\Html\RteHtmlParser`.
...
TYPO3 v8, magically adding `<p>` tags or transforming `<typolink>` tags is not
necessary anymore, which leaves transformations almost obsolete.
...
Activating CKEditor’s table plugin allows to add `<table>`, `<tr>` tags etc.
Enabling the link picker enables the usage of `<a>` tags. CKEditor cleans
content right away which was e.g. copy-pasted from word and does not
...
Mostly due to historical reasons, the frontend output added `<p>` tags to each
line which is not wrapped in HTML. Additionally the `<typolink>` tag was replaced
by `<a>` tags and checked if e.g. if a link was set to a specific page within
...
the stdWrap property `parseFunc`. With Fluid Styled Content and CSS Styled Content
comes `lib.parseFunc` and `lib.parseFunc_RTE` which add support for parsing `<a>`
and `<link>` tags and dumping them into the typolink functionality. The
shipped TypoScript code looks like this:
...
be achieved by adding `<f:format.html>{record.myfield}</f:format.html>`
which in turn calls stdWrap.parseFunc with `lib.parseFunc_RTE` thus
applying the same logic. Just ensure that the `lib.parseFunc_RTE`
...
to see if `lib.parseFunc_RTE` is filled.
